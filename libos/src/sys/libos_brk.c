...
int init_brk_region(void* brk_start, size_t data_segment_size) {
    ...
    if (brk_start && brk_start <= g_pal_public_state->private_user_address_end
            ...
            && (uintptr_t)brk_start < (uintptr_t)g_pal_public_state->private_user_address_end - brk_max_size) {
        int ret;
        size_t offset = 0;
        ...
        if (!g_pal_public_state->disable_aslr) {
            ...
            offset %= MIN((size_t)0x2000000, (size_t)((char*)g_pal_public_state->private_user_address_end -
