...
int init_vma(void) {
    ...
    if (!g_pal_public_state->disable_aslr) {
        ...
        size_t gap_max_size =
            (g_pal_public_state->private_user_address_end - g_pal_public_state->private_user_address_start) / 6 * 5;
...
int bkeep_mmap_any_aslr(size_t length, int prot, int flags, struct libos_handle* file,
    ...
    int ret;
    ret = bkeep_mmap_any_in_range(g_pal_public_state->private_user_address_start, g_aslr_addr_top, length,
