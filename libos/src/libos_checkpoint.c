...
static int send_memory_on_stream(PAL_HANDLE stream, struct libos_cp_store* store) {
    ...
    while (entry) {
        ...
        if (!(mem_prot & PAL_PROT_READ) && mem_size > 0) {
            ...
            ret = PalVirtualMemoryProtect(mem_addr, mem_size, mem_prot | PAL_PROT_READ, mem_prot);
            if (ret < 0) {
    ...
    if (hdr->mem_entries_cnt) {
        ...
        for (; entry; entry = entry->next) {
            ...
            if (!(prot & PAL_PROT_WRITE)) {
                ret = PalVirtualMemoryProtect(addr, size, prot, prot | PAL_PROT_WRITE);
                if (ret < 0) {
